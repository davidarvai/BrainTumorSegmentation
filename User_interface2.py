...
# Import the layout definitions from a separate file.
...
# Global variable for storing the path to the uploaded data folder
...
# Global CSS reset in the index_string to set html and body margin/padding to 0.
...
# ZIP file upload handling mechanism
@app.callback(
...
def handle_zip_upload(contents, filename):
    ...
    try:
        ...
        # After a successful upload: only the 'Continue to the system' link is displayed
        return uploaded_base_path, dcc.Link(
...
# 3D-s nézethez szükséges sequence dropdown frissítése
@app.callback(
...
# Loading slices for the 2D view
@app.callback(
